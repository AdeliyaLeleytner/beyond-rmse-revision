...
out=OUT/'data_repair';out.mkdir(exist_ok=True)
basepath=ROOT/'toxicodynamics/data/df_final.csv';srcpath=ROOT/'Toxicodynamics-aware-data/processed/prepared_admet.csv';rawpath=ROOT/'tmp/reviewer1_audit/admet_source_history.csv'
base=pd.read_csv(basepath);src=pd.read_csv(srcpath);pub=pd.read_csv(REPO/'data/df_final.csv')
...
audit=pd.DataFrame(records);audit.to_csv(out/'identity_manifest.csv',index=False)
...
pub_audit.to_csv(out/'published_association_audit.csv',index=False)
...
recovered.to_csv(out/'candidate_full_recovery_not_primary.csv',index=False)
recovered[recovered.identity_status.eq('unresolved_structure_transform')][['ligand_id','Cleaned SMILES','ADMET_source_SMILES']].to_csv(out/'unresolved_identity.csv',index=False)
