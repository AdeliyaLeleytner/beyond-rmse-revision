...
# Snapshot consultation provenance; advice is distinct from verified experimental evidence.
advice=json.loads((HERE.parents[2]/'tmp/reviewer1_audit/recalculation_advice.json').read_text())['_consultation']
dump(HERE/'consultation_record.json',{'independent_round':advice,'adopted_and_verified':['exact original prediction reproduction','fixed-test training-analogue removal with matched random reductions','biological localization controls'],'not_adopted':['automatic local conditional-coverage claims','treating inclusion of nominal coverage in a confidence interval as proof'],'followup':{'id':'20260906T222832Z-8d94d422','valid_advice_delivered':False,'reason':'wrapper budget limit','subject':'new evidence of ADMET identity misalignment'},'decision':'Use only structurally verified compound–ADMET pairs; run repaired and strict-identity sensitivity; revise scientific claims from measured results.'})
