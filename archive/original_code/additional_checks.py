...
PREV=HERE.parent/'recalculation'
...
def main():
    df=pd.read_csv(PREV/'data_repair/audited_parent_matched.csv').set_index('ligand_id')
    ...
    for seed in range(42,47):
        rd=PREV/f'repaired/runs/butina04_seed{seed}';config=json.loads((rd/'complete.json').read_text())
        ...
        groot=PREV/f'repaired/global_models/seed{seed}'
    ...
    grid=pd.read_csv(PREV/'repaired/tables/figure5_grid.csv');maps={'grid_cells':len(grid),'mean_E_min':float(grid.mean_E.min()),'mean_E_max':float(grid.mean_E.max()),'BTox_min':float(grid.phenotype_burden.min()),'BTox_max':float(grid.phenotype_burden.max()),'mask_neff_below':120,'panels':{c:{'masked_n':int(grid[c].isna().sum()),'masked_fraction':float(grid[c].isna().mean()),'h':.75 if c=='coverage' else .45} for c in ['mean','variance','width','coverage']}}
