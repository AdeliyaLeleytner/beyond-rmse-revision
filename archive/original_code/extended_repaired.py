...
def strict_sensitivity():
    df=pd.read_csv(HERE/'data_repair/strict_exact_identity.csv');a.OUT=HERE/'repaired/strict_identity';a.OUT.mkdir(exist_ok=True)
    ids=set(df.ligand_id)
    for seed in range(42,47):
        m=pd.read_csv(HERE/f'repaired/runs/butina04_seed{seed}/split_manifest.csv');m=m[m.ligand_id.isin(ids)]
...
def global_models(df_override=None,model_names=None):
    ...
    source=(a.ROOT/'tmp/reviewer1_audit/notebook_text/02_model_training.py').read_text()
    adme=ast.literal_eval(re.search(r'ADMET_FEATURES = (\[.*?\])',source,re.S).group(1))
