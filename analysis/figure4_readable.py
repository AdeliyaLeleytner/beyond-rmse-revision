...
import os
out=Path(os.environ.get('REVISION_FIGURES_OUT',ROOT/'results/figures'));out.mkdir(exist_ok=True)
...
for name in ['Baseline','BBB_pass','ADME']:
    a=np.load(RESULTS/f'global_models/seed42/{name}/shap_values.npz');sv=a['shap'][:,:-1];x=a['values'];features=a['features'];ranks=np.argsort(np.abs(sv).mean(0))[-20:]
